...
public class Utils {
   ...
   */
  public static <K> K requireNonNull(K o) {
    if (o == null) {
  ...
  /**
   * Gets the next string in this controller's scanner object only if the next input is a string.
   *
   * @return the next input in the scanner if it is a string, or if that string is a variation of
   *         "quit", quit the editor
   * @throws IllegalArgumentException if the next input is not a string
   */
  public static String getNextString(Scanner sc) throws IllegalArgumentException {
    Utils.requireNonNull(sc);
    if (sc.hasNext()) {
      String s = sc.next();
      if (s.equalsIgnoreCase("quit")) {
        ...
        try {
          Integer.parseInt(s);
          throw new IllegalArgumentException("Given string is a number");
      ...
      throw new IllegalArgumentException("Next input is not an string");
    }
  }

  /**
   * Gets the next integer in this controller's scanner object only if the next input can be
   * interpreted as an integer.
   *
   * @return the next input in the scanner if it is an int
   * @throws IllegalArgumentException if the next input is not an integer
   */
  public static int getNextInt(Scanner sc) throws IllegalArgumentException {
    Utils.requireNonNull(sc);
    if (sc.hasNextInt()) {
      return sc.nextInt();
    } else {
      throw new IllegalArgumentException("Next input is not an integer");
    }
  }
}
